...
const {
  Employees,
} = require("../../functions/Associations/employeeAssociations");
...
routes.post("/edit", async (req, res) => {
  ...
  try {
    ...
    await SE_Job.update(data, { where: { id: data.id } }).catch((x) =>
      console.log(1)
    );
    await SE_Equipments.destroy({ where: { SEJobId: data.id } }).catch((x) =>
      console.log(2)
    );
    await SE_Equipments.bulkCreate(createEquip(data.equipments, data.id)).catch(
      (x) => console.log(x)
    );
...
routes.post("/upsertLoadingProgram", async (req, res) => {
  try {
    const result = await Loading_Program.upsert(req.body).catch((x) =>
      console.log(x)
    );
    res.json({ status: "success", result: result });
    console.log(req.body);
